## Calcul de la puissance rayonnée effective (ERP)

* Ne prendre en compte que l'énergie qui arrive réellement à l'antenne – les pertes de câble sont déduites
...
### Conseils de calcul pour l'ERP

* Les pertes sont soustraites de la puissance d'émission avant d'appliquer le facteur de gain ($G_{antenne}$)
* Le référentiel par rapport à un dipôle demi-onde doit être respecté dans le calcul

---
[question:AG502]
---

### ERP en radioamateurisme – exemple pratique

* Le plan de fréquences pour la bande des $\qty{630}{\meter}$ fixe une ERP maximale de $\qty{1}{\watt}$
* Un dipôle demi-onde aurait une longueur de $\qty{315}{\meter}$ à $\qty{630}{\meter}$ – généralement irréalisable, donc des antennes raccourcies sont utilisées
* Les antennes raccourcies ont un rendement inférieur, par exemple un gain d'antenne de $\qty{-20}{\dBd}$
* Rapport de puissance : $\qty{-20}{\dB}$ correspond à un facteur de $\num{0,01}$ ; exemple : $\qty{50}{\watt} \cdot 0,01 = \qty{0,5}{\watt}$ d'ERP
...
| $\qty{10}{\dB}$ | $\num{10}$  | $\num{3,16}$ |
| $\qty{20}{\dB}$ | $\num{100}$ | $\num{10}$ |
[table:Pegel_Verhältnis:Rapports de puissance et de tension pour des valeurs importantes d'atténuation et d'amplification]
