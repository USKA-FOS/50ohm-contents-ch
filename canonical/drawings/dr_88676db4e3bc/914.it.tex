...
\begin{circuitikz}
    ...
    % Labels:
    \draw(amp1.south) node[anchor=north](){BF};
    \draw(gen1.east)  node[anchor=west](){CO};
    \draw(gen2.east)  node[anchor=west](){VFO};
    \draw(amp3.north) node[anchor=south](){\footnotesize PA};
    \draw(amp2.north) node[anchor=south](){\footnotesize Driver};
    \draw(mix2.north) node[anchor=south](){\footnotesize mixer};
    \draw(mix1.north) node[anchor=south](h1){\footnotesize bilanciato};
    \draw(h1.north)   node[anchor=south](){\footnotesize modulatore};
\end{circuitikz}