...
\begin{circuitikz}
    ...
    \draw[DARCblue](mix1.east)
        to [amp, box, >, l_=1.IF, name={amp2}] ++ (2,0) coordinate(f3);
    \draw(f3)
        node[inputarrow](){}
        node[mixer, box, anchor=west, DARCred] (mix2) {};
    \draw[DARCblue](mix2.east)
        to [amp, box, >, l_=2.IF, name={amp3}] ++ (2,0) coordinate(f4);
    \draw(f4)
        node[inputarrow](){}
        node[mixer, box, anchor=west, DARCred](mix3){};
    \draw[DARCblue](mix3.east)
        to [amp, box, >, l_=BF, name={amp4}] ++ (2,0) coordinate(f5);
    \draw(f5)
        node[inputarrow](){}
    ...
    \draw[DARCred](mix1.north) node[anchor=south](){1. mixer};
    \draw[DARCred](mix2.north) node[anchor=south](){2. mixer};
    \draw[DARCred](mix3.north) node[anchor=south west](){detettore di prodotto};
\end{circuitikz}