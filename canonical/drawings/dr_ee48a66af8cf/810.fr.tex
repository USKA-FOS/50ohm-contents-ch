...
\begin{circuitikz}
    ...
    \draw[DARCblue](mix1.east)
        to [amp, box, >, l_=1.FI, name={amp2}] ++ (2,0) coordinate(f3);
    \draw(f3)
        node[inputarrow](){}
        node[mixer, box, anchor=west, DARCred] (mix2) {};
    \draw[DARCblue](mix2.east)
        to [amp, box, >, l_=2.FI, name={amp3}] ++ (2,0) coordinate(f4);
    \draw(f4)
        node[inputarrow](){}
    ...
    \draw[DARCred](mix1.north) node[anchor=south](){1. mélangeur};
    \draw[DARCred](mix2.north) node[anchor=south](){2. mélangeur};
