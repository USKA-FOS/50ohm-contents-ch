...
\begin{circuitikz}
    ...
    \draw(sw.out 1)
        ...
        to [piezoelectric,/tikz/circuitikz/bipoles/length=0.7cm, >] ++(0,-0.5)
        node [rground](){};
    \draw(bp1.north) node[anchor=south](){1. filtro};
    ...
    \draw(mix.north) node[anchor=south](){mixer};
    \draw(bp2.north) node[anchor=south](){2. filtro};
    \draw(bp2.south) node[anchor=north](){\qty{28}-\qty{30}{\mega\hertz}};
    \draw(h1.center) node[anchor=east](){$f_\mathrm{o}$};
\end{circuitikz}