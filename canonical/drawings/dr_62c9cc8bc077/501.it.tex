...
\begin{circuitikz}
    ...
    \draw(n3.east)
        to[short] ++(1,0)
        node [antenna](ant){};
    \draw(h1.north) ++(0,0.75) node[anchor=south](h2){\footnotesize{e di controllo}};
    \draw(h2.north)            node[anchor=south](h3){\footnotesize{Segnali di comando}};
    \draw(h3.north)            node[anchor=south](h4){\footnotesize{Audio (BF) TX}};
    \draw(h4.north)            node[anchor=south](h5){\footnotesize{Audio (BF) RX}};
\end{circuitikz}