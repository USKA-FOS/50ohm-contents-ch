Le décibel est une unité de mesure logarithmique auxiliaire pour représenter les niveaux ou les rapports. Pour calculer un niveau, une référence est nécessaire, celle-ci est ensuite indiquée derrière le $\unit{\dB}$. Des exemples de cette écriture se trouvent dans le recueil de formules pour les niveaux de puissance et de tension :
$\qty{1}{\milli\watt}  \rightarrow  \unit{\dBm}$
$\qty{1}{\watt}  \rightarrow  \unit{\dBW} $
$\qty{0,775}{\volt}  \rightarrow  \unit{\dBu} $
L'unité de mesure auxiliaire $\unit{\dB\micro\volt\per\meter}$ signifie donc que la référence est de $\qty{1}{\micro\volt\per\meter}$.

Nous commençons donc par la formule :
