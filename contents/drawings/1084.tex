...
\begin{tikzpicture}
    ...
    % Phaseninkrement +2
    % N = 32 Tabelleneinträge, pro Takt wird die Phase um 2 erhöht
    \addplot[
        const plot,
        samples at={0,1,...,64},
        DARCred,
        ...
        (round(sin(360*2*x/32) * 1.5/2 * 7.5 + 0.5) - 0.5) / 7.5 * 2
    };

    \node[anchor=north west, DARCred] at (axis cs:2,2.7) {$\varphi_{n+1} = \varphi_n + 2$};
