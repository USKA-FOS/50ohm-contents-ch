...
## 7a. Use Case: Prepare A Versioned Site Release

### Goal

Produce one reproducible, complete multilingual release tree and promote it to
the dedicated Git release repository without publishing it automatically.
...
- `--release-id` cannot be combined with `--language` or `--skip-build`;
- a failed build or validation leaves the release repository unchanged;
- source Git states are verified again after generation;
- promotion never creates a commit, tag, or push;
- the release repository tag must match the public release id;
- the glossary is not recorded as a build input because existing translations
  are not regenerated by a glossary change.
